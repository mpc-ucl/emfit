...
if ks < kl % if small t is better...
    ...
    for k = -floor((K-1)/2):ceil((K-1)/2) % loop over k
        ...
        p2_dz = p2_dz + w*(1-w)*exp(-((w + 2*k)^2)/2/tt)+(w+2*k)*w*(1-w)*(-2)*(w+2*k)*exp(-((w + 2*k)^2)/2/tt);
    end
    ...
    for k = 1:K
        ...
        p2_dz = p2_dz+k*exp(-(k^2)*(pi^2)*tt/2)*(k*pi*w*(1-w))*cos(k*pi*w);
    end
...
p1_dz = (-v*a*w*(1-w))*exp(-v*a*w -(v^2)*t/2)/(a^2);
