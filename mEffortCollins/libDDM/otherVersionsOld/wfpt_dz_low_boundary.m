function p = wfpt_dz_low_boundary(t,v,a,z,err)
...
wl = 1-w; % starting point is swapped for low choice --> because only swap 
% here, need to get regular starting point as input to this function 
...
if ks < kl % if small t is better...
    ...
    for k = -floor((K-1)/2):ceil((K-1)/2) % loop over k
        p2 = p2 + (wl + 2*k)*exp(-((wl + 2*k)^2)/2/tt); % increment sum
        p2_dz = p2_dz + (-1)*w*(1-w)*exp(-((wl + 2*k)^2)/2/tt)+(wl+2*k)*(-1)*w*(1-w)*2*(wl+2*k)*exp(-((wl + 2*k)^2)/2/tt);
    end
    p2 = p2/sqrt(2*pi*tt^3); % add constant term
    p2_dz = p2_dz/sqrt(2*pi*tt^3); % add constant term
else % if large t is better...
    K=ceil(kl); % round to smallest integer meeting error
    for k = 1:K
        p2 = p2+k*exp(-(k^2)*(pi^2)*tt/2)*sin(k*pi*wl); % increment sum
        p2_dz = p2_dz+k*exp(-(k^2)*(pi^2)*tt/2)*(k*pi*(-1)*w*(1-w))*cos(k*pi*wl);
    end
...
p1 = exp(-v*a*wl -(v^2)*t/2)/(a^2);
p1_dz = (-v*a*(-1)*w*(1-w))*exp(-v*a*wl -(v^2)*t/2)/(a^2);
