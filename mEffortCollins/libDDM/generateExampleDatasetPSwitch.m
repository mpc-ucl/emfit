function Data=generateExampleDatasetPSwitch(Nsj,resultsDir,allResults)
% 
...
for sj=1:Nsj; 
	...
	Data(sj).trueParam = [0 0 0 0 0 0]'+.6*randn(6,1);
    %Data(sj).trueParam = allResults(sj).llreweffscalingDDMBSPPSwitchSimple.params;
    
    %Data(sj).trueParam = rand(5,1)*8-4';
	%  generate surrogate behavioural data 
	[foo,foo,dsurr] = llreweffscalingDDMBSPPSwitchEmfit(Data(sj).trueParam,Data(sj),0,0,0,options); 
	Data(sj).a = dsurr.a;
    Data(sj).decisiontime = dsurr.simTime;  
	Data(sj).trueModel='llreweffscalingDDMBSPPSwitchEmfit';
