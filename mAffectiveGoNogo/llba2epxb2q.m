function [l,dl,dsurr] = llba2epxb2q(x,D,mu,nui,doprior,options);
% 
...
% towards one action, irreducible noise, two separate positive Pavlovian bias
% parameter for rewards and losses, and two different q0 values for win and loss
% stimuli 
% 
...
for t=1:length(a)
	...
	if dodiff
		...
		if rho
			dl(7) = dl(7) + g*(p0(a(t)) * (dQdq(a(t),s(t),1) - p0'*dQdq(:,s(t),1))) / pg(a(t));
			dQdq(a(t),s(t),1) = (1-alfa)*dQdq(a(t),s(t),1) ;
		else
			dl(8) = dl(8) + g*(p0(a(t)) * (dQdq(a(t),s(t),2) - p0'*dQdq(:,s(t),2))) / pg(a(t));
