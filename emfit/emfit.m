...
Np = length(x);
beta = exp(x(1));            % compute sensitivity to reward / noise from transformed parameter 
eps = 1./(1+exp(-x(2)));     % learning rate from transformed parameter 

% apply prior with mean musj and variance nui or not 
if doprior
...
for t=1:length(D.A)
	% compute choice likelihood given parameter 
	l0 = max(Q);
	...
	er = beta * D.R(t);	% effective reward 
